        ...
        try:
            ...
            if force == 'yes' or force == 'Yes' or force == 'y' or force == 'Y': 
                ...
                for one_env in all_env:
                    ...
                    if rule_name and customer_acc:
                        if one_env.name ==  rule_name:
                            deployment_id =  self.get_deployment_ids(one_env.config.env_id, customer_acc, api_instance)
                    elif customer_acc:
                        deployment_id =  self.get_deployment_ids(one_env.config.env_id, customer_acc, api_instance)
                ...
                for deployment_id in deployment_id_to_remove:
                    DestroyDeployment.destroy_by_deploymentid(self, deployment_id)
    ...
    def  get_deployment_ids(self, env_id, customer_acc, api_instance):
        ...
        if all_deployment:
            for single_deployment in all_deployment:
                if single_deployment.input_json and customer_acc in single_deployment.input_json:
