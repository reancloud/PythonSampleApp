...
class SaveProvider(Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument('--name', '-n', help='Set name for Provider', required=True)
        parser.add_argument('--type', '-t', help='Type of provider', required=True)
        parser.add_argument('--input_json_file', '-f', help='Json File with applicable key-value pair for provider type', required=True)
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        try:
            ...
            provider = swagger_client.SaveProvider(name=parsed_args.name,
                type=parsed_args.type,json=jsondata)
